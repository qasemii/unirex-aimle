...
from src.model.select_k import SelectK, my_calc_expl
...
class LanguageModel(BaseModel):
    ...
    def calc_attrs(self, input_ids, attn_mask, targets, rationale=None, noisy_rationale=None):
        ...
        # Compute attrs via LM
            ...
            # attrs are score of tokens in the given inputs
        ...
        if self.l2e:
            ...
            l2e_attrs = None
            # just keep non-pad tokens score and set the others to zero
    ...
    def forward(self, inputs, attention_mask, mode='task', attrs=None):
        ...
        if mode == 'task' and self.a2r and attrs is not None:
            ...
            if self.dataset == 'cose':
                # logits are scores of output classes for each input
            ...
            if self.dataset == 'cose' and self.l2e:
                ...
                # logits are score of each token in each input
        elif mode == 'captum':
    ...
    def run_step(self, batch, split, batch_idx):
        ...
        # here I think we can change `expl_reg_freq` for more explanations regularization
